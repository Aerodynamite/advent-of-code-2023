...
long[] ParseSeeds(string seeds) => seeds.Split(": ").Last().Split(" ").Select(long.Parse).ToArray();
...
class Almanac
{
    ...
    public Almanac(long[] seeds, AlmanacMap seedToSoil, AlmanacMap soilToFertilizer, AlmanacMap fertilizerToWater,
